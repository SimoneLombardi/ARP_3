...
int main(int argc, char *argv[])
{
    ...
    fdo_s[0] = fd_unpack[2][0]; // obstacle --> socket server
    fdo_s[1] = fd_unpack[2][1];

    fdss_s_t[0] = fd_unpack[3][0]; // socket server --> server/ target
    fdss_s_t[1] = fd_unpack[3][1];

    closeAndLog(fdss_s_t[0], "socket server: close fdss_s_t[0]" ):


    fdss_s_o[0] = fd_unpack[4][0]; // socket server --> server/ obstacle
    ...
    fds_ss[0] = fd_unpack[5][0]; // server --> socket server/window size
                ...
                if (FD_ISSET(fd_array[i], &read_fd))
                {
                    ...
                    if (fd_array[i] == fdt_s[0]) // <<<< target - socket_server >>>>
                    {
                        ...
                        else
                        {
                            ...
                            if (retVal_write < 0)
                            {
                                ...
                                writeLog("==> ERROR ==> socket_server: write fdt_s[1], %m ");
                    ...
                    else if (fd_array[i] == fdo_s[0]) // <<<< obstacle - socket_server >>>>
                    {
                        ...
                        if (retVal_read == -1)
                        {
                            ...
                            writeLog("==> ERROR ==> socket_server: read fdo_s[0], %m ");
                            exit(EXIT_FAILURE);
                        }
                        else
                        {
